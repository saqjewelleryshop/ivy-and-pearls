...
function PaymentStep({orderNumber}){const stripe=useStripe(),elements=useElements();const [busy,setBusy]=useState(false),[error,setError]=useState('');
 async function pay(e){e.preventDefault();if(!stripe||!elements)return;setBusy(true);setError('');const {error}=await stripe.confirmPayment({elements,confirmParams:{return_url:`${window.location.origin}/order-confirmed/?order=${encodeURIComponent(orderNumber)}`}});if(error){setError(error.message);setBusy(false)}}
...
export default function Checkout(){
 const cart=useCart(),{user,session}=useAuth();const [step,setStep]=useState('details'),[clientSecret,setClientSecret]=useState(''),[orderNumber,setOrderNumber]=useState(''),[error,setError]=useState(''),[busy,setBusy]=useState(false),[stripePromise,setStripePromise]=useState(null),[stripeConfig,setStripeConfig]=useState(null);
 ...
 useEffect(()=>{let live=true;api('/stripe/config').then(config=>{if(!live)return;setStripeConfig(config);if(config.enabled&&config.publishableKey)setStripePromise(loadStripe(config.publishableKey));}).catch(e=>{if(live)setError(e.message)});return()=>{live=false}},[]);
 ...
 async function createIntent(e){e.preventDefault();setBusy(true);setError('');try{
  if(!stripeConfig?.enabled)throw new Error('Payments are currently unavailable.');
 ...
 {step==='details'?<form className="checkout-form" onSubmit={createIntent}><div className="form-grid"><label>Email<input type="email" required {...field('email')}/></label><label>Phone<input required {...field('phone')}/></label><label>First name<input required {...field('first_name')}/></label><label>Last name<input required {...field('last_name')}/></label><label className="span-2">Address<input required {...field('address1')}/></label><label className="span-2">Address line 2<input {...field('address2')}/></label><label>Town / city<input required {...field('city')}/></label><label>County<input required {...field('province')}/></label><label>Postcode<input required {...field('postcode')}/></label><label>Country<select value="GB" disabled><option value="GB">United Kingdom</option></select></label><label className="span-2">Order note<textarea rows="3" {...field('customerNote')}/></label></div><button className="button button--dark" disabled={busy||!stripeConfig?.enabled}>{busy?'Preparing payment…':stripeConfig?.enabled?'Continue to secure payment':'Payments unavailable'}</button>{error&&<p className="form-error" role="alert">{error}</p>}</form>:
